...
final class TermDTO_Test extends DatabaseTestBase
{
    ...
    public function test_smoke_test_simple_dto() {  // V3-port: DONE tests/unit/term/test_repo
    ...
    public function test_dto_gets_tags_as_array()  // V3-port: DONE tests/unit/term/test_repo
    {
        $english = new Language();
        $english->setLgName('English');
    ...
    public function test_buildTerm_returns_existing_term_in_language()  // V3-port: DONE sort of in unit/term/test_repo
    {
        $t = new Term();
    ...
    public function test_buildTerm_with_new_parent_text_creates_new_parent()  // V3-port: DONE test unit term test_repo
    {
     ...
     * @group dtoparent
     */
    public function test_buildTerm_with_new_parent_parent_gets_translation_and_image_and_tag()  // V3-port: DONE test unit term test_repo
    {
        foreach(['perros', 'perro'] as $text) {
            $f = $this->term_service->find($text, $this->spanish);
            $this->assertTrue($f == null, 'Term not found at first');
     ...
     * @group dtoparent
     */
    public function test_cannot_set_dto_term_as_its_own_parent()  // V3-port: DONE unit term test_repo
    {
        $dto = new TermDTO();
        $dto->language = $this->spanish;
        $dto->Text = 'perro';
     ...
     * @group dtoparent
     */
    public function test_add_term_existing_parent_creates_link() {  // V3-port: DONE test unit term test_repo
        $p = new Term();
     ...
     * @group dtoparent
     */
    public function test_add_term_existing_parent_parent_gets_translation_if_missing() {  // V3-port: DONE test unit term test_repo
    ...
    public function test_buildTerm_returns_new_term_if_no_match() {  // V3-port: DONE in unit term test_repo
        $dto = new TermDTO();
        $dto->language = $this->spanish;
        $dto->Text = 'perro';
    ...
    public function test_buildTerm_returns_tags_as_TermTags() {  // V3-port: DONE not applicable?
        $dto = new TermDTO();
        $dto->language = $this->spanish;
        $dto->Text = 'perro';
        $dto->termTags[] = 'hi';
    ...
    public function test_buildTerm_returns_tags_as_existing_TermTags() {  // V3-port: DONE in test/unit/term/test_repo
    ...
    public function test_remove_tags_from_dto_removes_them_from_saved_Term() {  // V3-port: DONE test/unit/term/test_repo
    ...
    // problems.
    public function test_creating_save_term_with_new_parent_and_tags_works()  // V3-port: DONE in test unit term test_repo
    {
        $dto = new TermDTO();
        $dto->language = $this->spanish;
        $dto->Text = 'perros';
